$(function () {
  ...
  $.getJSON(url, function(result) {
    ...
    var entries = result.feed.entry.map( function (entry) {
      ...
      card.find('.dictionary__grammar').text(entry.gsx$grammatica.$t);
      ...
      entry.gsx$ancora.$t != "" ? card.find('.dictionary__anchor').attr("name", entry.gsx$ancora.$t.toLowerCase()) : null;
